...
import org.pabuff.evs2helper.mqtt.PagCommAgentHelper;
...
public class AcControllerOperator {
    ...
    public String pagCommAgentPgprPath;
...
//                }
    ...
    public Map<String, Object> submitServiceOnOff2(Map<String, Object> meterInfo, AcControllerStatus desiredAcControllerStatus, SvcClaimDto svcClaimDto) {

        String meterSn = (String) meterInfo.get("meter_sn");
        if(meterSn == null) {
            logger.warning("meter_sn is null");
            return Map.of("error", "meter_sn is null");
        }

        String siteTag =  (String) meterInfo.get("site_tag");
        if(siteTag == null) {
            logger.warning("site_tag is null");
            return Map.of("error", "site_tag is null");
        }
        ...
        if(mock){
            ...
            try {
                ...
                String topicPublish;
                String pagCommAgentPath;
                switch (siteTag) {
                    case "nus_vh":
                        topicPublish = "evs2/nus/vh/" + gw;
                        pagCommAgentPath = pagCommAgentVhPath;
                        break;
                    case "nus_pgpr":
                        topicPublish = "evs2/nus/pgpr" + block + "/" + gw;
                        pagCommAgentPath = pagCommAgentPgprPath;
                        break;
                    default:
                        topicPublish = null;
                        pagCommAgentPath = null;
                }

                if(topicPublish == null) {
                    logger.warning("topicPublish is null");
                    return Map.of("error", "topicPublish is null");
                }

                String mid = UUID.randomUUID().toString();
                Map<String, Object> payload = new LinkedHashMap<>();
                payload.put("pca_path", pagCommAgentPath);
                payload.put("topic_pub", topicPublish);
                payload.put("meter_sn", meterSn);
                payload.put("cmd", acControllerCommand);
                payload.put("mid", mid);
                ...
                QueryReqDto<Map<String, Object>> req = new QueryReqDto<>(payload);
                StdRespDto stdRespDto = pagCommAgentHelper.pcaR(cred, req, "/evs2/submit_ac_controller_command");
...
//                }
    ...
    public Map<String, Object> checkAcControllerStatus2(Map<String, Object> meterInfo, SvcClaimDto svcClaimDto){
        ...
        if(mock){
            ...
            payload.put("pca_path", pagCommAgentPath);
            payload.put("topic_pub", topicPublish);
            payload.put("topic_sub", topicSubscribe);
            payload.put("meter_sn", meterSn);
            payload.put("cmd", "check_service_status");
            payload.put("get_response", "true");
            payload.put("mid", UUID.randomUUID().toString());
